...
```
  ...
  * For each sensor that's returned:
      ...
      * Do an AQI calculation (this _could_ be done inside of ThingsBoard, if that approach is preferred)
      * Send all sensor data for that device (based on the "access token") to ThingsBoard
```

## Widgets
### Map
* Administrators will use this widget as a way to see individual sensor locations across the Portland/Gresham area.
